...
func TestCanSignEdgeCases(t *testing.T) {
...
func TestCanSignSSHWithoutKey(t *testing.T) {
	tmpDir := t.TempDir()
	repo := setupRepository(t, tmpDir, false)

	// Set SSH format but don't set signing key
	err := repo.SetGitConfig("gpg.format", "ssh")
	require.NoError(t, err)

	// This should return an error because SSH requires a signing key
	err = repo.CanSign()
	assert.NotNil(t, err)
	assert.Equal(t, ErrSigningKeyNotSpecified, err)
}

func TestCanSignWithSSHAndKey(t *testing.T) {
	tmpDir := t.TempDir()
	repo := setupRepository(t, tmpDir, false)
...
func TestCanSignDefaultBehavior(t *testing.T) {
	tmpDir := t.TempDir()
	repo := setupRepository(t, tmpDir, false)

	// No signing configuration set - should default to GPG which doesn't require key
	err := repo.CanSign()
	assert.Nil(t, err)
}

func TestCanSignWithVariousFormats(t *testing.T) {
